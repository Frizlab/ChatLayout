...
public final class ChatLayout: UICollectionViewLayout {
	...
	/** Default `UIScrollView` behaviour is to keep content offset constant from the top edge.
	 If this flag is set to `true` `ChatLayout` should try to compensate batch update changes to keep the current content at the bottom of the visible part of `UICollectionView`.
	
	 - Note: Keep in mind that if during the batch content inset changes also (e.g. keyboard frame changes),
	 `ChatLayout` will usually get that information after the animation starts and wont be able to compensate that change too.
...
#warning("Pinning is not checked. If we snapshot to a pinned item, this would not mean anything.")
	...
	public override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
		 ...
		 * At this time, `ChatLayout` has no way of knowing the details of this data source count change (where the insert/delete/move took place).
		 * `ChatLayout` only gets this additional information once `prepareForCollectionViewUpdates:` is invoked.
