...
struct LayoutModel {
	...
	mutating func setAndAssemble(header: Item, sectionIndex: Int) {
	...
	mutating func setAndAssemble(item: Item, sectionIndex: Int, itemIndex: Int) {
		guard sectionIndex < sections.count else {
			assertionFailure("Incorrect section index.")
			return
		}
	...
	mutating func setAndAssemble(footer: Item, sectionIndex: Int) {
		guard sectionIndex < sections.count else {
			assertionFailure("Incorrect section index.")
			return
		}
	...
	mutating func insertSection(_ section: Section, at sectionIndex: Int) {
	...
	mutating func insertItem(_ item: Item, at indexPath: IndexPath) {
		sections[indexPath.section].insert(item, at: indexPath.item)
		resetCache()
	}
	
	mutating func replaceItem(_ item: Item, at indexPath: IndexPath) {
