...
enum ChangeItem : Equatable {
	...
	init?(with updateItem: UICollectionViewUpdateItem) {
		...
		switch updateAction {
			...
			case .move:
				guard
					let indexPathBeforeUpdate = indexPathBeforeUpdate,
					let indexPathAfterUpdate = indexPathAfterUpdate
				else {
	...
	private var rawValue: Int {
		switch self {
			case .sectionReload: return 0
			case .itemReload:    return 1
			case .sectionDelete: return 2
			case .itemDelete:    return 3
			case .sectionInsert: return 4
			case .itemInsert:    return 5
			case .sectionMove:   return 6
			case .itemMove:      return 7
		}
	}
